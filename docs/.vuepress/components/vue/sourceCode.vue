<template>
  <div class="demo">
    ...
    <ul>
      ...
      <li>编译器：用来将模板字符串编译成js渲染函数的代码，体积大，效率低</li>
      <li>
        运行时（runtime）:
        仅包含运行时,不包含编译器,创建Vue实例、渲染并处理虚拟DOM等的代码，体积小，效率高，是轻量级版本
      </li>
      <li>CommonJS: cjs规范,用于配合老的打包工具比如：webpack1</li>
      <li>ES Module: ES模块,用于webpack2</li>
      <li>
        UMD:
    ...
    <p>进入Vue的_init方法，我们可以看到内部又包含了很多初始化的过程</p>
    ...
    <ul>
      <li>
        ...
        <ul>
          ...
          <li>注册了Vue.compile()方法，传递一个HTML字符串返回render函数</li>
        </ul>
      </li>
      <li>
        ...
        <ul>
          ...
          <li>
            全局方法：
            <ul>
              <li>__patch__:把虚拟DOM转换成真实DOM</li>
              <li>$mount: 挂载方法</li>
            </ul>
          </li>
        </ul>
      ...
      <li>
        ...
        <ul>
          <li>与平台无关</li>
          <li>设置了Vue的静态方法，initGlobalAPI(Vue)</li>
        </ul>
      </li>
      <li>
        ...
        <ul>
          <li>与平台无关</li>
          <li>定义了构造函数，调用了this._init(options)方法</li>
          <li>与Vue中混入了常用的实例成员</li>
        </ul>
      </li>
    </ul>
    ...
    <p>这里我们要回想一下vue的渲染过程是通过renderWatcher来实现的</p>

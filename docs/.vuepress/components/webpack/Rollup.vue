<template>
  <div>
    ...
    <p>Rollup 诞生的目的并不是要与 Webpack 这样的工具全面竞争。它的初衷只是希望能够提供一个高效的 ES Modules 打包器，充分利用 ES Modules 的各项特性，构建出结构扁平，性能出众的类库。</p>
    <p>至于它的其他特点和优势，我们需要上手过后才能深入了解。</p>
    <h3>快速上手</h3>
    <p>这里我准备了一个简单的示例，具体结构如下：</p>
    <Content slot-key="code-rollup1" />
    <p>在这个示例的源代码中我准备了三个文件，并且使用 ES Modules 组织的代码模块化。部分代码如下：</p>
    <Content slot-key="code-rollup2" />
    <p>如上述代码所示，其中：</p>
    <ul>
    ...
    <p>$ npx rollup --config # 使用默认配置文件</p>
    <p>$ npx rollup --config rollup.prod.js # 指定配置文件路径</p>
    <h3>输出格式</h3>
    <p>Rollup 打包支持多种输出格式，这里我们回到配置文件中，配置同时输出所有格式下的文件，具体配置如下：</p>
    <Content slot-key="code-rollup5" />
    ...
    <img :src="$withBase('/webpack/rollup4.png')" />
    ...
    <img :src="$withBase('/webpack/rollup5.png')" />
    ...
    <p>为了抹平这个差异，Rollup 给出了一个 @rollup/plugin-node-resolve 插件，通过使用这个插件，我们就可以在代码中直接使用模块名称导入模块了。</p>
    <p>同样，我们需要先安装这个插件，具体命令如下：</p>
    <p>$ npm i @rollup/plugin-node-resolve --save-dev</p>
    <p>安装完成过后，打开配置文件，这里同样导入插件函数，然后把它配置到 plugins 数组中。具体配置如下：</p>
    <Content slot-key="code-rollup8" />
    <p>完成以后我们就可以回到代码中直接导入 node_modules 中的第三方模块了。例如：</p>
    <Content slot-key="code-rollup9" />
    <p>这里我导入的是我提前安装好的一个 lodash-es 模块，这个模块就是常用的 lodash 模块的 ESM 版本。导入过后我们就可以使用这个模块所提供的工具方法了。</p>
    ...
    <Content slot-key="code-rollup14" />
    <p>这样的话，再次打包输出的结果就是采用 AMD 标准组织的代码了，具体如下：</p>
    <img :src="$withBase('/webpack/rollup9.png')" />
    <p>需要注意一点，这种 AMD 标准在浏览器中也不是直接支持的，也就是说我们还是需要使用一个支持这个标准的库来加载这些输出的模块，例如 Require.js，具体使用方式参考：</p>
    <p>P.S. 本文中所有的案例源代码：https://github.com/zce/rollup-demo</p>
    <h3>总结</h3>
    ...
    <p>$ npm install parcel-bundler --save-dev</p>
    ...
    <img :src="$withBase('/webpack/Parcel1.png')" />
    ...
    <img :src="$withBase('/webpack/Parcel1.gif')" />
    ...
    <Content slot-key="code-Parcel5" />
    <p>然后回到 main.js 中通过 import 导入这个样式文件，具体如下：</p>
    <Content slot-key="code-Parcel6" />
    <p>保存过后，样式文件可以立即生效。效果如下：</p>
    <img :src="$withBase('/webpack/Parcel2.png')" />
    ...
    <p>P.S. Webpack 中也可以使用一个叫作 happypack 的插件实现这一点。</p>
    <p>那我们这里再来看一下输出的打包结果，具体结果如下：</p>

    <img :src="$withBase('/webpack/Parcel4.png')" />
